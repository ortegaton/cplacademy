...
class chart_3dpie extends propsbyparam{
    ...
    private function _setpropsbasedondata(){
        ...
        if ($xtra > 0) $this->_xtra_height = (5+15*$antal_label)-($this->height+ceil($this->shadow_height));

        $this->_xtra_width = 5;
        if ($this->show_label) $this->_xtra_width += 20;
        if ($this->show_percent) $this->_xtra_width += 45;
        if ($this->show_text) $this->_xtra_width += $this->_text_length*8;
        if ($this->show_parts) $this->_xtra_width += 35;
    ...
    private function _genimage(){
        $img = ImageCreateTrueColor($this->width+$this->_xtra_width, $this->height+ceil($this->shadow_height)+$this->_xtra_height);
        ...
        for ($i = 0; $i < count($this->label); $i++) 
        {
        	...
        	{
        		imagefilledellipse($img,$this->width+11,$label_place+5,10,10,$this->_colorHex($img, $this->colors[$i % count($this->colors)]));
        		imageellipse($img,$this->width+11,$label_place+5,10,10,$this->_colorHex($img, $this->text_color));
        	}
        	else if ($this->label_form == 'square' && $this->show_label)
        	{	
        		imagefilledrectangle($img,$this->width+6,$label_place,$this->width+16,$label_place+10,$this->_colorHex($img, $this->colors[$i % count($this->colors)]));
        		imagerectangle($img,$this->width+6,$label_place,$this->width+16,$label_place+10,$this->_colorHex($img, $this->text_color));
        	}
        	...
        	imagestring($img,'4',$this->width+20,$label_place,$this->label_output,$this->_colorHex($img, $this->text_color));
        	$this->label_output = '';
