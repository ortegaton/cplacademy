...
class mis_time {
     ...
     * @return integer UTS
     */
    function adjust_time($time, $years=0, $months=0, $days=0, $hours=0, $minutes=0, $seconds=0){
        $y=date('Y', $time);
        $m=date('n',$time);
        $d=date('j',$time);
        $th=date('G', $time);
     ...
     * @return integer UTS
     */
    function first_of_month($time=false){
        $time=$time ? $time : time();
     ...
     * @return integer uts
     */	
    static function last_date_of_month($month=false, $year=false){
     ...
     */
    static function mon_of_week($tm=false){
        $tm=$tm===false ? time() : $tm;
     ...
     * @return integer uts 
     */
    static function first_mon_of_month($month=false, $year=false){
        $month=$month===false ? date('m') : $month;
        $year=$year===false ? date('Y') : $year;        
	 ...
	 */
	function strtotime_yyyymmdd($str, $sep='-'){
		$darr=explode('-', $str);
	 ...
	 * @return object
	 */
    static function dectime_to_hoursmins($dectime){
