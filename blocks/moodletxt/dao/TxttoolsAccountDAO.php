...
class TxttoolsAccountDAO {
     ...
     * @param string $url URL of the ConnectTxt server
     * @return boolean True if account exists
     * @version 2015062901
     * @since 2012060101
     */
    public function accountNameExists($accountName, $url) {
        global $DB;
        
        return ($DB->count_records('block_moodletxt_accounts', array('username' => $accountName, 'url' => $url)) > 0);
    }
     ...
     * @since 2015062901
     */
    public function getAccountByLocation($location){
        global $DB;

        $accounts = array();
        $resultSet = $DB->get_records('block_moodletxt_accounts', array('location' => $location));

        foreach($resultSet as $result)
            $accounts[$result->id] = $this->convertStandardClassToBean($result);
    ...
    /**
     * returns url, location of the existing accounts
     *
     * @return stdClass object containing two fields: location, url
     * @version 2015070915
     * @since 2015070915
     */
    public function getUrlAndLocation() {
        global $DB;
        return $DB->get_record('block_moodletxt_accounts', array(), 'url, location');
    }
