...
class MoodletxtAddressbookDAO {    
    ...
    public function getAddressbookById($addressbookId, $userId, $includeContacts = false, $includeGroups = false) {
        ...
        if ($includeContacts)
            $constructedAddressbook->setContacts(
                $this->getAddressbookContactsForUser($userId, $addressbookRecord->id));
        
        if ($includeGroups) {
            $groups = array($this->getAddressbookGroupsForUser($userId, $addressbookRecord->id));
            $constructedAddressbook->setGroups($groups);
        }
    ...
    /**
     * @return string[] set of all the phone numbers from user profiles and address book contacts
     * @version 2015062901
     * @since 2015062901
     */
    public function getPhonenumbersOfAllContacts(){
        global $DB;

        $results = $DB->get_records('block_moodletxt_ab_entry', null, '', 'phoneno');
        $phonenumbers = array();
        foreach($results as $result){
            $phonenumbers[] = $result->phoneno;
        }
