...
function handleLoadedUserRestrictionData(json) {
    ...
    var $outboundCell = $accountRow.find('td:nth-child(5)');
...
function handleAccountAccess(json) {
    ...
    var tableRow = activeRequests[json.accountID];
    var $accountRow = $accountsTable.find('tr:nth-child(' + tableRow + ')');
    var $outboundCell = $accountRow.find('td:nth-child(6)');
    var $inboundCell = $accountRow.find('td:nth-child(7)');
...
function updateAllAccounts(event) {
    ...
    // There are empty rows in the table with the .emptyrow class -> skip these to prevent useless http requests
    if ($accountsTable.children('thead').length > 0) {
        $tableRowSet = $accountsTable.find('tbody tr:not(.emptyrow)');
        tableFudge = 1;
    } else {
        $tableRowSet = $accountsTable.find('tr:not(:first-child):not(.emptyrow)');
        tableFudge = 2;
    }
...
function handleAccountInfoUpdate(json) {
    ...
    if (json.hasError) {
        ...
        $accountRow.find('td:nth-child(8)').text(json.creditsUsed).css('font-weight','bold');
        $accountRow.find('td:nth-child(9)').text(json.creditsRemaining).css('font-weight', 'bold');
        $accountRow.find('td:nth-child(10)').text(json.billingType).css('font-weight', 'bold');
        $accountRow.find('td:nth-child(11)').text(json.updateTimeString).css('font-weight', 'bold');
    }
...
$(document).ready(function() {
    ...
    $accountsTable.find('td:nth-child(5) img:first-child').each(function(index) {
        $(this).click(function(event) {
            loadAccountDialog(event, handleLoadedUserRestrictionData);
        });
    });
   ...
   // Toggle outbound access when clicked
    $accountsTable.find('td:nth-child(6) img:last-child').each(function(index) {
        $(this).click(toggleOutboundAccess);
    });

    // Toggle inbound access when clicked
    $accountsTable.find('td:nth-child(7) img').each(function(index) {
