...
class MoodletxtXMLParser {
    ...
    function __construct() {
        ...
        $this->targetElements['/' . MoodletxtXMLConstants::$RESPONSE_ROOT . '/' . MoodletxtXMLConstants::$RESPONSE_OPT_OUT_CHECK_BLOCK]   = 'buildOptOutCheckMessage';
...
//        error_log(get_string('logxmlparsedobjects', 'block_moodletxt') . "\r\n\r\n" . print_r($returnObjects, true));
    ...
    /**
     * Builds AccountDetails objects from inbound XML
     * @param SimpleXMLElement $node The captured elements to build from
    ...
    /**
     * Builds buildOptOutCheckMessage objects from parsed XML
     * @param SimpleXMLElement $node The captured elements to build from
     * @return array of statuses keyed by phone numbers
     * @version 2015062901
     * @since 2015040201
     */
    private function buildOptOutCheckMessage($node) {
        file_put_contents('C:\\moodle.log', time() . ': parsing optout response ', FILE_APPEND);

        $receivedOptOutResponses = array();

        // Check that required children exist for object build
        if (isset($node->{MoodletxtXMLConstants::$RESPONSE_OPT_OUT_BLOCK})) {
            foreach($node->children() as $response) {
                foreach($response->attributes() as $name => $value){
                    if($name === MoodletxtXMLConstants::$RESPONSE_OPT_OUT_BLOCK_FOR)
                        $phoneNumber = (string) $value;
                }

                $status = (string) $response;
                if(isset($phoneNumber) && $status === 'BLACK')
                    $receivedOptOutResponses[$phoneNumber] = $status;
            }
        }
        
        return $receivedOptOutResponses;
    }
