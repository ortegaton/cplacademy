...
if ($formData != null) {
    ...
    try {
        // if this is the first account being added get connection details from the form
        if($accountDAO->countTxttoolsRecords() < 1){
            $location = $formData->accountCtxtInstance;
            if($location === 'URL') {
                $url = $formData->accountUrl;
            }else if($location === TxttoolsAccount::$US_LOCATION) {
                $url = TxttoolsAccount::$US_URL;
            }else if($location === TxttoolsAccount::$UK_LOCATION) {
                $url = TxttoolsAccount::$UK_URL;
            }
        ...
        } else if($formData->accountCtxtInstance !== TxttoolsAccount::$UK_LOCATION && $formData->accountCtxtInstance !== TxttoolsAccount::$US_LOCATION
            && ($formData->accountCtxtInstance !== 'URL' && $formData->accountUrl)) {
