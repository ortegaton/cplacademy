...
if (! $table->is_downloading()) {
    ...
    $topSegment = $output->box($messageDL, 'generalbox float-left w50');
    ...
    $statusKey .= get_string('labelstatusdelivered', 'block_moodletxt') . html_writer::empty_tag('br');

    $statusKey .= $output->render(new moodletxt_icon(moodletxt_icon::$ICON_STATUS_BLACKLISTED,
        get_string('altstatusblacklisted', 'block_moodletxt')));
    $statusKey .= get_string('labelstatusblacklisted', 'block_moodletxt');

    $topSegment .= $output->box($statusKey, 'generalbox float-left w50');
    echo($output->box($topSegment, 'float-left w100'));
