...
class class_view extends ZiLinkPluginBase implements iZiLinkBlockPlugin {
    ...
    public function GetBlockContent($block) {
        ...
        if (isloggedin()) {
            if (($COURSE->id <> SITEID) &&
                ($this->person->Security()->IsAllowed('local/zilink:class_view'))) {

                $students = $this->data->GetLinkedPeopleData('students', 'details', context_course::instance($COURSE->id));
                if ($students) {
                    ...
                    $content->items[] = html_writer::link(new moodle_url($this->httpswwwroot .
                                        '/local/zilink/plugins/class/view/interfaces/' .
                                        $CFG->zilink_class_view_interface . '/pages/index.php',
                                        array('sesskey' => sesskey(), 'cid' => $COURSE->id)),
                                        get_string('class_view_class', 'local_zilink'));
                }
            } else {
    ...
    public function SetTitle() {
        return get_string('class_view', 'local_zilink');
    }
