...
class guardian_view extends ZiLinkPluginBase implements iZiLinkBlockPlugin {
    ...
    public function GetBlockContent($block) {
        ...
        if (isloggedin()) {
            if ($this->person->Security()->IsAllowed('local/zilink:guardian_view')) {
                $children = $this->data->GetLinkedPeopleData('children', 'details');
                if ($children) {
                    $count = 0;
                    foreach ($children['children'] as $idnumber => $child) {
                        $content->icons[] = '<img src="' . $OUTPUT->pix_url('/i/user') . '" class="icon" alt="" />';
                        $content->items[] = html_writer::link(new moodle_url($this->httpswwwroot .
                        '/local/zilink/plugins/guardian/view/interfaces/'
                        . $CFG->zilink_guardian_view_interface . '/pages/index.php',
                        array('sesskey' => sesskey(), 'offset' => $count)),
                        fullname($DB->get_record('user', array('idnumber' => $idnumber))));
                        $count++;
                    }
                }
            } else {
    ...
    public function SetTitle() {
        return get_string('guardian_view', 'local_zilink');
    }
