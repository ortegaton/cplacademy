...
;(function (jQueryRMC, window, document) {
	...
	function Plugin(element, options) {
		...
		this.$el = jQueryRMC(element);
		this.elProxyPosition;
		// this will be the element which triggers the appearance of the tooltip on hover/click/custom events.
		// it will be the same as this.$el if icons are not used (see in the options), otherwise it will correspond to the created icon
		this.$elProxy;
		this.enabled = true;
		this.options = jQueryRMC.extend({}, defaults, options);
	...
	Plugin.prototype = {
			...
			if (document.querySelector) {
				...
				if ((!deviceHasTouchCapability && self.options.iconDesktop) || (deviceHasTouchCapability && self.options.iconTouch)) {
					...
					if(typeof self.options.icon === 'string'){
						...
						self.$elProxy = jQueryRMC('<span class="'+ self.options.iconTheme +'"></span>');
		...
		showTooltipNow: function() {
			...
			if (self.enabled && self.content !== null) {
				...
				if (self.options.onlyOne) {
					jQueryRMC('.tooltipstered').not(self.$el).each(function(i,el) {
				...
				self.options.functionBefore(self.$elProxy, function() {
					...
					// if the tooltip isn't already open, open that sucker up!
						...
						self.bodyOverflowX = jQueryRMC('body').css('overflow-x');
						jQueryRMC('body').css('overflow-x', 'hidden');
						...
						self.$tooltip = jQueryRMC('<div class="tooltipster-base '+ self.options.theme +'" style="'+ fixedWidth +' '+ maxWidth +' '+ pointerEvents +' '+ animationSpeed +'"><div class="tooltipster-content"></div></div>');
						...
						jQueryRMC(window).on('scroll.'+ self.namespace +' resize.'+ self.namespace, function() {
							self.positionTooltip();
						});
						...
						if (self.options.autoClose) {
							...
							jQueryRMC('body').off('.'+ self.namespace);
							...
							if (self.options.trigger == 'hover') {
								...
								if (deviceHasTouchCapability) {
									...
									setTimeout(function() {
										...
										jQueryRMC('body').on('touchstart.'+ self.namespace, function() {
											self.hideTooltip();
										});
									}, 0);
								}
							...
							// here we'll set the same bindings for both clicks and touch on the body to hide the tooltip
								...
								setTimeout(function() {
									jQueryRMC('body').on('click.'+ self.namespace +' touchstart.'+ self.namespace, function() {
										self.hideTooltip();
									});
								}, 0);
		...
		setCheckInterval: function(){
			...
			self.checkInterval = setInterval(function() {
				...
				if (
						...
						jQueryRMC('body').find(self.$el).length === 0
						// if the elProxy has been removed
					||	jQueryRMC('body').find(self.$elProxy).length === 0
						// if the tooltip has been closed
					||	self.status == 'hidden'
						// if the tooltip has somehow been removed
					||	jQueryRMC('body').find(self.$tooltip).length === 0
				) {
		...
		hideTooltip: function() {
			...
			if (self.status == 'shown' || self.status == 'appearing') {
				...
				var finish = function() {
					...
					jQueryRMC(window).off('.'+ self.namespace);
		...
		positionTooltip: function() {
			...
			if (jQueryRMC('body').find(self.$tooltip).length !== 0) {
				...
				var arrowReposition = null,
					windowWidth = jQueryRMC(window).width(),
					// shorthand
				...
				function dontGoOffScreenX() {
					...
					var windowLeft = jQueryRMC(window).scrollLeft();
				...
				function dontGoOffScreenY(switchTo, switchFrom) {
					...
					if(((proxy.offset.top - jQueryRMC(window).scrollTop() - tooltipHeight - offsetY - 12) < 0) && (switchFrom.indexOf('top') > -1)) {
						practicalPosition = switchTo;
					}
					
					// if it goes off the bottom of the page
					if (((proxy.offset.top + proxy.dimension.height + tooltipHeight + 12 + offsetY) > (jQueryRMC(window).scrollTop() + jQueryRMC(window).height())) && (switchFrom.indexOf('bottom') > -1)) {
						practicalPosition = switchTo;
	...
	jQueryRMC.fn[pluginName] = function () {
			...
			if (typeof args[0] === 'string') {
				...
				switch (args[0]) {
					...
					case 'setDefaults':
						...
						jQueryRMC.extend(defaults, args[1]);
						break;
		...
		// this happens when calling $(sel).tooltipster('methodName or options') where $(sel) matches one or more elements
			...
			if (typeof args[0] === 'string') {
				...
				this.each(function() {
					...
					var self = jQueryRMC(this).data('tooltipster');
			...
			// first argument is undefined or an object : the tooltip is initializing
				...
				return this.each(function () {
					...
					if (!jQueryRMC(this).data('tooltipster')) {
						jQueryRMC(this).data('tooltipster', new Plugin( this, args[0] ));
					}
				});
			}
		}
	...
	function areEqual(a,b) {
		var same = true;
		jQueryRMC.each(a, function(i, el){
	...
	jQueryRMC('body').one('mousemove', function() {
		deviceHasMouse = true;
	});
