...
class zilink_student_reporting_opened_reports_form extends moodleform {
    function definition() {
        ...
        $mform->setType('tid',PARAM_INT);
         ...
         $mform->addElement('html', '<table class="generaltable boxaligncenter" width="80%">
                                            ...
                                            <th class="header c0" style="text-align:center;" scope="col">'.get_string('report_writer_report_opened', 'local_zilink').'</th>
                                            </tr>
                                            </thead>
                                            <tbody>
                                            <tr class="r0">');
        ...
        if (isset($this->_customdata['rows']) && !empty($this->_customdata['rows'])) {
                ...
                foreach ($this->_customdata['rows'] as $row) {
                    ...
                    if ($row[4] <> -1) {
                        ...
                        $mform->addElement('advcheckbox', 'opened['.$row[5].']','','',array('group'=> 2), array(0,1));
                            if ( $row[4] == 1) {
            ...
            $mform->addElement('html', '<tr class="r1">');
            $mform->addElement('html', '<td class="cell c0" style="text-align:left;">No Reports Available To Be Opened</td>');
