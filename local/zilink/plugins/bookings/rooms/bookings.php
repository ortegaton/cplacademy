...
$bookedroom = optional_param('zilink_bookings_rooms_booked_room','',PARAM_RAW);
...
$urlparams = array('course' => $courseid, 'sesskey' => $sesskey);
$PAGE->https_required();
$PAGE->set_url('/local/zilink/plugins/bookings/rooms/bookings.php', $urlparams);
$PAGE->verify_https_required();


if(empty($courseid) || empty($sesskey))
    redirect($CFG->httpswwwroot,'',0);
...
$tt_url = new moodle_url('/local/zilink/plugins/bookings/rooms/bookings.php', $urlparams);
...
try {
    ...
    $booking  = new ZiLinkBookingManager($courseid);
    
    if(!empty($dayid) && !empty($periodid) && empty($selecteddates) && empty($bookedroom))
    {
       $content = $booking->RoomBookingBookingForm(array('dayid' => $dayid, 'periodid' => $periodid));     
       
    }
    elseif(!empty($selecteddates) && !empty($bookedroom))
    {
        if($booking->RoomBookingCreateBooking($dayid,$periodid,$selecteddates,$bookedroom))
        {
            $content =  $booking->RoomBookingGetTimetable();
            $content .=  $booking->RoomBookingGetCurrentBookings();
        }
    }
    else {
        $content = $booking->RoomBookingGetTimetable();
